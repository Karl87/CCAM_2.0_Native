...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section ==1 && _noMoreData == NO) {
        ...
        if (![_topComment.userID isEqualToString:[[AuthorizeHelper sharedManager] getUserID]]) {
            [self.textView becomeFirstResponder];
            [self.textView setPlaceholder:[NSString stringWithFormat:@"回复%@:",_topComment.userName]];
            self.replyName = _topComment.userName;
            self.replyID = _topComment.userID;
        }else{
            [self.textView setPlaceholder:Babel(@"说点什么吧...")];
            self.replyName = @"";
            self.replyID = @"";
        ...
        if (![comment.userID isEqualToString:[[AuthorizeHelper sharedManager] getUserID]]) {
            [self.textView becomeFirstResponder];
            [self.textView setPlaceholder:[NSString stringWithFormat:@"回复%@:",comment.userName]];
            self.replyName = comment.userName;
...
- (void)didPressRightButton:(id)sender{
    ...
    NSString *commentReplyID = self.replyID.copy;
    NSString *commentReplyName = self.replyName.copy;
...
//    NSDictionary *parameters = @{@"workid":_photoID,@"comment":comment,@"token":[[AuthorizeHelper sharedManager] getUserToken]};
    ...
    NSDictionary *parameters = @{@"workid":_photoID,@"comment":comment,@"token":[[AuthorizeHelper sharedManager] getUserToken],@"replymname":commentReplyName,@"replymemberid":commentReplyID};

    NSLog(@"%@",parameters);
    [manager POST:CCamSendReplyURL parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"%@",[[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding]);
        ...
        commentObj.replyUserID = commentReplyID;
        commentObj.replyUserName = commentReplyName;
...
- (void)deleteCommentWith:(NSIndexPath *)indexPath{
    ...
    NSDictionary *parameters = @{@"cid":comment.commentID};
    [manager POST:CCamDeleteCommentURL parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"%@",[[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding]);
        ...
        NSOrderedSet *timelineCommentSet = [_timeline.comments copy];
        for (CCComment * co in timelineCommentSet) {
            if (![comment.commentID isEqualToString:co.commentID]) {
                [timelineComments addObject:co];
            }
        }
        [_timeline setComments:[[NSOrderedSet alloc] initWithArray:timelineComments]];
