...
@interface EraseTutorialViewController ()
...
@implementation EraseTutorialViewController
...
- (void)viewDidLoad {
    ...
    for (int i=0; i<30; i++) {
        if (i<10) {
            NSString*fileName =[NSString stringWithFormat:@"CC__0000%d",i];
            NSString *filePath = [[NSBundle mainBundle] pathForResource:fileName ofType:@"png"];
            UIImage *image = [UIImage imageWithContentsOfFile:filePath];
            [_tutorialImages addObject:image];
        }else{
            NSString*fileName =[NSString stringWithFormat:@"CC__000%d",i];
            NSString *filePath = [[NSBundle mainBundle] pathForResource:fileName ofType:@"png"];
            UIImage *image = [UIImage imageWithContentsOfFile:filePath];
            [_tutorialImages addObject:image];
        }
...
- (void)startTutorial{
    ...
    [TutorialHelper sharedManager].autoShowEraserTutorial = NO;
    [[TutorialHelper sharedManager] setShowEraserTutorial:@"NO"];
}
- (void)endTutorial{
    [_tutorial stopAnimating];
    [[TutorialHelper sharedManager] dismissEraseTutorialView];
}
- (void)dealloc{
