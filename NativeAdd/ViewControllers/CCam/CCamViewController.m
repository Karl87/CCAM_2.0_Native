...
@property (nonatomic,strong) SEFilterControl *eraseSlider;
...
//    [self.view setUserInteractionEnabled:NO];
    ...
    if ([userGroup isEqualToString:versionGroup]) {
        ...
        if ([DataHelper sharedManager].filters.count == 0) {
            [[DataHelper sharedManager] getLocalFilters];
            [[DataHelper sharedManager] updateFiltersInfo];
        }
    }else{
        [[DataHelper sharedManager] updateSeriesInfo];
        [[DataHelper sharedManager] updateStickerSetsInfo];
...
- (void)callControlTutorial{
    [[TutorialHelper sharedManager] callControlTutorialView];
}
- (UIView *)setNavigationView{
    ...
    _navigationHelpBtn = [UIButton new];
    [_navigationHelpBtn setFrame:CGRectMake(0, 0, CCamThinNaviHeight, CCamThinNaviHeight)];
    [_navigationHelpBtn setImage:[UIImage imageNamed:@"helpIcon"] forState:UIControlStateNormal];
    [_navigationHelpBtn setCenter:CGPointMake(CCamViewWidth/2+50, CCamThinNaviHeight/2)];
    [_navigationHelpBtn addTarget:self action:@selector(callControlTutorial) forControlEvents:UIControlEventTouchUpInside];
    [_navigationView addSubview:_navigationHelpBtn];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    if (collectionView.tag == CCamSegmentCollectionView) {
        ...
        return [[DataHelper sharedManager].filters count];//[_filters count];
    }
    return 0;
}
...
//                UIView *sliderBG;
                if (!_eraseSlider) {
                    if (isPad) {
                        _eraseSlider = [[SEFilterControl alloc] initWithFrame:CGRectMake(0, 0, segContentCell.bounds.size.width/2-30, 10) Titles:@[@"1x",@"2x",@"3x",@"4x",@"5x"]];

                        [_eraseSlider setCenter:CGPointMake(segContentCell.bounds.size.width-(_eraseSlider.bounds.size.width)/2-60, (segContentCell.bounds.size.height-CCamThinSerieHeight)/2)];
                    }else{
                        _eraseSlider = [[SEFilterControl alloc] initWithFrame:CGRectMake(0, 0, segContentCell.bounds.size.width - 88, 10) Titles:@[@"1x",@"2x",@"3x",@"4x",@"5x"]];

                        [_eraseSlider setCenter:CGPointMake(segContentCell.bounds.size.width/2, segContentCell.bounds.size.height-2*CCamThinSerieHeight)];
                    }
                    [segContentCell.contentView addSubview:_eraseSlider];
                    [_eraseSlider addTarget:self action:@selector(getSliderValue:) forControlEvents:UIControlEventTouchUpInside];
                    [_eraseSlider setProgressColor:CCamViewBackgroundColor];
                    [_eraseSlider setTopTitlesColor:CCamRedColor];
                    [_eraseSlider setSelectedIndex:2];
...
//                [segContentCell.eraseBrushSlider setValue:2 animated:YES];
                ...
                [segContentCell.eraseHelpButton setCenter:CGPointMake(_eraseSlider.center.x+_eraseSlider.frame.size.width/2+10, _eraseSlider.center.y+5)];
...
//                [serieContentCell.surfaceView.surfaceButton setHidden:NO];
        ...
        CCFilter * filter = (CCFilter*)[[DataHelper sharedManager].filters objectAtIndex:indexPath.row];
        if (indexPath.row == 0) {
            [filterCell.filterLabel setText:filter.name_file];
            [filterCell.filterLabel setFont:[UIFont boldSystemFontOfSize:17.0]];

        }else{
            [filterCell.filterLabel setText:[NSString stringWithFormat:@"#%ld",(long)indexPath.row]];
            [filterCell.filterLabel setFont:[UIFont boldSystemFontOfSize:21.0]];
        }
...
//        }
        [filterCell.filterImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",CCamHost,filter.image_mini]]];
...
- (void)getSliderValue:(id)sender{
    SEFilterControl *slider = (SEFilterControl*)sender;
    self.eraseSize = (float)slider.SelectedIndex;
    [self setEraseOptions];
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [_serieContentCollection scrollRectToVisible:CGRectMake(_serieContentCollection.bounds.size.width*indexPath.row, 0, _serieContentCollection.bounds.size.width,  _serieContentCollection.bounds.size.height) animated:NO];
        ...
        if (indexPath.row == 0) {
            [_navigationHelpBtn setHidden:NO];
        }else{
            [_navigationHelpBtn setHidden:YES];
        }
...
//        NSString* filter = [_filters objectAtIndex:indexPath.row];
        CCFilter *filter = (CCFilter*)[[DataHelper sharedManager].filters objectAtIndex:indexPath.row];
        NSString* filterName = filter.name_file;
        UnitySendMessage(UnityController.UTF8String, "OnSelectFilter", filterName.UTF8String);
    }else if (collectionView.tag == CCamStickerSetCollectionView) {
