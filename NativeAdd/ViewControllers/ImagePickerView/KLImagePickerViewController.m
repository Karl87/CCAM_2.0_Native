...
@interface KLImagePickerViewController ()<UIScrollViewDelegate,UITableViewDataSource,UITableViewDelegate,FastttCameraDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDelegate,UICollectionViewDataSource,UIImagePickerControllerDelegate,UINavigationControllerDelegate>{
    BOOL captureRunning;
    BOOL fitSize;
}
...
//        KLCaptureViewController *capture = [[KLCaptureViewController alloc] init];
...
//    ShowImageViewController *show =[[ShowImageViewController alloc] init];
    ...
    if (fitSize) {
        if (![[iOSBindingManager sharedManager] saveCroppedImageWith:[self normalizedImage:[self getMixImage:_albumImg.image color:_albumImg.backgroundColor]]]) {
            return;
        }
    }else{
        if (![[iOSBindingManager sharedManager] saveCroppedImageWith:[self normalizedImage:[self getCroppedImageWith:_albumImg.image]]]) {
            return;
        }
    }
...
- (void)flashButtonPressed
{
    ...
    if (self.fastCamera.cameraFlashMode == FastttCameraFlashModeOn) {
        flashMode = FastttCameraFlashModeOff;
        flashTitle = @"captureFlashOffIcon";
    }else if (self.fastCamera.cameraFlashMode == FastttCameraFlashModeOff){
