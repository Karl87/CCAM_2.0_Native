...
-(CGPoint)getCenterPointForIndex:(int) i{
    return CGPointMake((i/(float)(titlesArr.count-1)) * (self.frame.size.width-RIGHT_OFFSET-LEFT_OFFSET) + LEFT_OFFSET, i==0?self.frame.size.height-25-TITLE_SELECTED_DISTANCE:self.frame.size.height-25);
}
...
-(id) initWithFrame:(CGRect) frame Titles:(NSArray *) titles{
    if (self = [super initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, 70)]) {
        ...
        [_handler setFrame:CGRectMake(LEFT_OFFSET, 8, 30, 30)];
        ...
        UIImage *image1 = [UIImage imageNamed:@"circlePoint"];
        UIImage *image2 = [UIImage imageNamed:@"circlePoint"];
        ...
        for (i = 0; i < titlesArr.count; i++) {
            ...
            [lbl setTextColor:CCamSwitchNormalColor];
