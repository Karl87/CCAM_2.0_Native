...
//            [_surfaceButton.imageView setContentMode:UIViewContentModeScaleAspectFit];
        ...
        if (!_surfaceProgress) {
            _surfaceProgress  = [[M13ProgressViewBorderedBar alloc] initWithFrame:CGRectMake(self.bounds.size.width/2-65, self.bounds.size.height/2 + 32, 175, 10)];
            [_surfaceProgress setProgressDirection:M13ProgressViewBorderedBarProgressDirectionLeftToRight];
            [_surfaceProgress setCornerType:M13ProgressViewBorderedBarCornerTypeCircle];
            [_surfaceProgress setPrimaryColor:CCamRedColor];
            [_surfaceProgress setSecondaryColor:CCamRedColor];
            [_surfaceProgress setSuccessColor:CCamRedColor];
            [_surfaceProgress setFailureColor:CCamRedColor];
...
- (void)dealloc{
    if (_surfaceProgress) {
        [_surfaceProgress setProgress:1.0 animated:NO];
    }
}
@end
