...
- (void)downloadCharacter:(CCCharacter *)character{
    ...
    NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        NSLog(@"%@",[NSString stringWithFormat:@"%f",(CGFloat)downloadProgress.fractionCompleted]);
        ...
        if ([DataHelper sharedManager].ccamVC.serieContentCollection) {
            ...
            UIView *characterProgress = [[DataHelper sharedManager].ccamVC.serieContentCollection viewWithTag:8000+[character.characterID intValue]];
            
            if (characterProgress) {
                if ([characterProgress isKindOfClass:[M13ProgressViewBorderedBar class]]) {
            ...
            UIView *surfaceProgress = [[DataHelper sharedManager].ccamVC.serieContentCollection viewWithTag:7000+[character.characterID intValue]];
            if (surfaceProgress) {
                if ([surfaceProgress isKindOfClass:[CCamSerieContentSurfaceView class]]) {
