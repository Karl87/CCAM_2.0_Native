    ...
    [manager POST:CCamLoginURL parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (jsonStr.length>3) {
            ...
            if (login) {
                ...
                NSString *stateMessage =GetValidString([jsonDic objectForKey:@"status"]) ;
                NSString *userToken =GetValidString([jsonDic objectForKey:@"token"]);
                NSString *userID = GetValidString([jsonDic objectForKey:@"memberid"]);
                NSString *groupID = GetValidString([jsonDic objectForKey:@"groupid"]);
                NSString *userZone = GetValidString([jsonDic objectForKey:@"country"]);
                NSString *userName = GetValidString([jsonDic objectForKey:@"membername"]);
                NSString *userImage = GetValidString([jsonDic objectForKey:@"head_img"]);
                ...
                NSLog(@"%@",userToken);
                [[AuthorizeHelper sharedManager] setUserToken:GetValidString(userToken)];
                [[AuthorizeHelper sharedManager] setUserID:GetValidString(userID)];
                [[AuthorizeHelper sharedManager] setUserGroup:GetValidString(groupID)];
                [[AuthorizeHelper sharedManager] setUserZone:GetValidString(userZone)];
                [[AuthorizeHelper sharedManager] setUserName:GetValidString(userName)];
                [[AuthorizeHelper sharedManager] setUserImage:GetValidString(userImage)];
