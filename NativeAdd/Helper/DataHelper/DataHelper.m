...
- (void)updateFiltersInfo{
    NSLog(@"Start request Filters info");
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer = [AFHTTPResponseSerializer serializer];
    
    [manager POST:CCamGetFiltersURL parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([responseObject length]==1) {
            ...
            //Delete old data
            [[CoreDataHelper sharedManager] deleteStoreInfoWithEntity:@"CCFilter"];
            ...
            for (int i = 0; i<[receiveArray count]; i++) {
                NSDictionary *tempFilter = [receiveArray objectAtIndex:i];
                CCFilter *filter = [NSEntityDescription insertNewObjectForEntityForName:@"CCFilter" inManagedObjectContext:context];
                [filter initFilterWith:tempFilter];
              
                if (![context save:&error]) {
                    NSLog(@"滤镜%@保存失败",filter.name_file);
                }
                tempFilter = nil;
            }
            receiveArray = nil;
        }
        [self getLocalFilters];
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        NSLog(@"Request stickers fail");
        [self getLocalFilters];
    }];
}
- (void)getLocalFilters{
    NSManagedObjectContext *context = [[CoreDataHelper sharedManager] managedObjectContext];
    NSFetchRequest *request = [[NSFetchRequest alloc] init];
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"CCFilter" inManagedObjectContext:context];
    [request setEntity:entity];
    NSError *error;
    NSArray *infos = [context executeFetchRequest:request error:&error];
    if ([infos count] == 0) {
        NSLog(@"本地无数据，需要联网同步数据");
        [self updateFiltersInfo];
        return;
    }else{
        NSLog(@"本地数据库读取%lu个滤镜",(unsigned long)infos.count);
    }
    ...
    [self.filters addObjectsFromArray:[context executeFetchRequest:request error:&error]];
    
    NSLog(@"=========>本地滤镜数%lu",(unsigned long)self.filters.count);
    
    [_ccamVC updateFilterCollection];
}
...
//            [_updateSeriesHud setLabelText:Babel(@"更新角色信息成功")];
            ...
            NSString *currentVersion = GetValidString([receiveDic objectForKey:@"version"]);
